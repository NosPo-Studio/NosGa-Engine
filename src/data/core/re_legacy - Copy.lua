...
local function isInsideArea(ra, go, cmi)
	...
	if cmi.raw.x > 0 then
		...
		--table.insert(ra.gameObjectAttributes[go].drawAreas, {x + sx, y + sy, x + sx, y + sy})
	...
	if ra.gameObjectAttributes[go] ~= nil then
		if not ra.firstRender then
			local x, y = x + sx, y + sy
			--global.log()
			--table.insert(ra.gameObjectAttributes[go].drawAreas, {x, y, x + sx, y + sy})
		end
	end
				...
				if --[[l >= layer and]] not isInQueue(renderArea, go, l) and renderArea.layerBlacklist[l] ~= true and isInsideArea(renderArea, go, renderArea.cameraMoveInstructions) == 1 then
					...
					then
						if renderArea.gameObjectAttributes[go] ~= nil then	
							global.log("TTTTTTTTT", gameObject.ngeAttributes.name)
							...
							print("[RE]: Found overlap with: " .. gameObject.ngeAttributes.name .. ": N:" .. go.ngeAttributes.name .. ", L:" .. tostring(l) .. ", X:" .. tostring(x2) .. ", Y:" .. tostring(y2) .. ", ID:"..  tostring(go) .. ", F:" .. tostring(global.currentFrame) .. "RA: " .. renderArea.name .. ".")
						end
					end
				end
			end
...
local function calculateFrame(renderArea, area)
	...
	for go in pairs(renderArea.gameObjects) do
		...
		local x, y = go:getPos()
		local sx, sy = go:getSize()
		local x2, y2 = x + sx, y + sy
		
		--table.insert(renderArea.gameObjectAttributes[go].drawAreas, {x, y, x2, y2})
...
function re.draw()
	...
	for ra in pairs(global.renderAreas) do
		if ra.visible then
			...
			clearFrame(ra)
			drawFrame(ra) --Have to be executed befor ra:ngeDraw().
			ra:ngeDraw()
			ra:resetCMI()
		end
	end
